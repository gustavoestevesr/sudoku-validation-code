...
function checkSudoku(matrix) {
    var tempRowsArr = new Set();
    var tempColsArr = new Set();
    var tempBoxesArr = new Map();
    for (var i = 0; i < 9; i++) {
        for (var j = 0; j < 9; j++) {
            ...
            if (matrix[i][j] === "." || matrix[j][i] === ".")
                continue;
            // Verify cols
            if (tempColsArr.has(matrix[j][i]))
                return false;
            tempColsArr.add(matrix[j][i]);
            // Verify rows
            if (tempRowsArr.has(matrix[i][j]))
                return false;
            tempRowsArr.add(matrix[i][j]);
            // Verify boxes 3x3
            var boxKey = Math.floor(i / 3) + ',' + Math.floor(j / 3);
            var boxValues = tempBoxesArr.get(boxKey) || "";
            if (boxValues.includes(matrix[i][j]))
                return false;
